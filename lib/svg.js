...
class SVG {
    ...
    generateSVGContent() {
        ...
        svgContent += `<text x="60" y="60" fill="${this.letterColor}">${this.letters}</text>`;
        return svgContent;
    }
    ...
    run() {
        inquirer.prompt([
            ...
            },
            {
                ...
                name: 'shapeColor',
                message: `'Choose a color for your shape. Press ENTER to view all color choices.' ${coloredTemplate}`,
                choices: colorChoices  // Provide color choices here
            },
            {
                type: 'list',
                name: 'letterColor',
                message: `'Choose a color for the letters.' ${coloredTemplate}`,
        ...
        .then((answers) => {
            if (answers.letters.length > 3) {
                ...
                fs.writeFile('shape.svg', `<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200">${svgContent}</svg>`, (err) => {
                    if (err) {
